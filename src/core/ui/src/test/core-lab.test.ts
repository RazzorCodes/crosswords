...
test('lab storage round-trips model buffers and reset clears state', () => {
  ...
  const state: LabState = {
    ...
    ledger: [sample('A', 1)],
    cnnArtifacts: {
      checkpoint: new Uint8Array([1, 2, 3]).buffer,
      inferenceModel: new Uint8Array([4, 5]).buffer,
      updatedAt: 100,
    },
  };
  ...
  assert.deepEqual(Array.from(new Uint8Array(loaded.cnnArtifacts?.checkpoint ?? new ArrayBuffer(0))), [1, 2, 3]);
...
test('model reset behavior is represented by deleting personalized snapshots', () => {
  ...
  const state: LabState = { ...emptyLabState, svmSnapshot: snapshot, cnnArtifacts: { updatedAt: 1 } };
...
test('WASM export smoke test sees core feature and SVM symbols', async (t) => {
  const wasmPath = resolve('public/wasm/handwriting_core.wasm');
