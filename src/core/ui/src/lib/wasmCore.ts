...
function readFloat64(ptr: number, length: number): Float64Array {
  return new Float64Array(requireExports().memory.buffer, ptr, length).slice();
}

function readUint32(ptr: number, length: number): Uint32Array {
  return new Uint32Array(requireExports().memory.buffer, ptr, length).slice();
}

function readUint8(ptr: number, length: number): Uint8Array {
  return new Uint8Array(requireExports().memory.buffer, ptr, length).slice();
}
...
export function extractFeatures(strokes: StrokeInput): number[] {
  ...
  try {
    ...
    return Array.from(readFloat64(outAlloc.ptr, FEATURE_COUNT));
  } finally {
    free(pointsAlloc);
    free(lengthsAlloc);
    free(outAlloc);
...
export function trainSvm(samples: LabSample[], readyLetters: string[]): LabSvmSnapshot | null {
  ...
  try {
    ...
    const labelIndices = readUint8(outLabelsAlloc.ptr, readyLetters.length);
    const labels = Array.from(labelIndices).map(indexToLabel);
    return {
...
export function predictSvm(features: number[], snapshot: LabSvmSnapshot): Record<string, number> {
  ...
  const biasesAlloc = mallocCopy(snapshot.biases);
  const startsAlloc = mallocCopy(snapshot.starts);
  const countsAlloc = mallocCopy(snapshot.counts);
  const coeffAlloc = mallocCopy(snapshot.coefficients);
  const supportAlloc = mallocCopy(snapshot.supportVectors);
  const meanAlloc = mallocCopy(snapshot.featureMean);
  const stdAlloc = mallocCopy(snapshot.featureStd);
  const outAlloc = mallocZeroed(ALPHABET.length * Float64Array.BYTES_PER_ELEMENT);
  try {
