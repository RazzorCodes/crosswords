...
import { estimateJsonBytes, formatBytes } from './storage.ts';
...
export async function recognizeLabStrokes(strokes: StrokeInput, state: LabState): Promise<LabRecognitionResult> {
  ...
  if (state.svmSnapshot) {
    ...
    const svmSize = estimateJsonBytes(state.svmSnapshot);
    engineResults.push({
      algorithm: 'svm',
      status: 'ready',
      ...
      detail: `${state.svmSnapshot.supportCount} SVs / ${formatBytes(svmSize)}`,
    });
  } else {
    engineResults.push({
      algorithm: 'svm',
  ...
  try {
    ...
    const cnnSize = state.cnnArtifacts 
      ? state.cnnArtifacts.modelSafetensors.byteLength + state.cnnArtifacts.optimizerState.byteLength 
      : state.baselineManifest?.cnn.trainingArtifacts.modelSize ?? 0;
    engineResults.push({
      algorithm: 'cnn',
      ...
      detail: cnn ? `${cnn.source} CNN${cnnSize ? ` / ${formatBytes(cnnSize)}` : ''}` : 'no CNN model in manifest',
    });
  } catch (error) {
    engineResults.push({
      algorithm: 'cnn',
